                    ...
                    if (scope.oncreated({
                        ...
                        type.on("mouseenter", function () {
                            var value, seriesName, self = $(this),
                                parent = self.parent();
                            "Pie" === scope.type ? (seriesName = parent.find(".ct-label").jsp(), value = "") : (scope.type = "Line") && (value = self.attr("ct:value"), seriesName = parent.attr("ct:series-name") || "", seriesName && (seriesName += " : ")), tooltip.jsp("<div class='tooltip-inner'>" + seriesName + value + "</div>").show()
...
!function () {
    ...
    angular.module("app.email.ctrls", []).controller("EmailCtrl", ["$scope", "$modal", function ($scope, $modal) {
        $scope.labelColors = ["#5974d9", "#19c395", "#fc3644", "#232429", "#f1d44b"], $scope.labels = [{
            ...
            $modal.open({
                templateUrl: "views/email/compose.jsp",
                size: "lg",
